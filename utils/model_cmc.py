...
from .convLSTM_upgrade import ConvLSTMCell
import copy
...
default_params = CMC_Params(
    img_shape=(240, 240),
    # 20180502  seq_len = 1 (images onlt)
    #sequence_length=3,
    sequence_length=1,
    num_classes=2,
)
...
class Model(object):
    ...
    def net(self, inputs, is_training=True):
        ...
        #print('input transpose:', images_seq.get_shape())
        images_seq = tf.unstack(images_seq)
        #print('unstack:', images_seq)
        ...
        for i in range(self.params.sequence_length):
            #print('image_seq shape:', images_seq)
            #print('range(self.params.sequence_length):', range(self.params.sequence_length))
            image_mod = tf.split(axis=3, num_or_size_splits=4, value=images_seq[i])
            #print('image_mod:', image_mod)
            for j in range(4):
                # share weight or not
        ...
        cell = ConvLSTMCell(start_channel, k_size=3, height=f_shape[1], width=f_shape[2], initializer=orthogonal_initializer()) #
